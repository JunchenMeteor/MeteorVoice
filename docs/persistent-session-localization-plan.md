...
- Keep an active voice conversation alive while users navigate between app pages.
- Preserve clear recording and speaking indicators everywhere in the app.
- Avoid silent background listening outside an explicitly active session.
...
- `Sidebar` currently protects accidental navigation, but it does not keep the voice engine alive after navigation.
...
- Browser microphone and audio playback behavior must remain explicit: the app should not record unless a user has started an active session.
...
- Own active session state: scenario key, accent key, messages, status, corrections, summary, current workflow snapshot, active turn id, and provider preference.
- Expose actions: `startSession`, `endSession`, `continueTurn`, `playCorrection`, and `selectScenario`.
- Keep refs used by async voice turns inside the provider so they survive route changes.
- Publish whether recording or speaking is active for global UI indicators.
...
- Listen, transcribe, call chat, play TTS, append assistant messages, and automatically start the next turn.
...
- Layout-level mini controller: show listening/speaking/paused state, current scenario, and End Session.
- Non-session pages: do not show full transcript by default, but keep the active session visible enough that users understand the app may be listening.
...
- The current voice loop keeps running only while the user explicitly started a session.
...
- Manually verify:
  ...
  - Navigate to Home/Settings and return to Session with transcript preserved.
...
- Browser STT implementations may stop when the tab loses focus or permissions change. The UI should show a recoverable paused/error state instead of pretending the app is still listening.
...
3. `[Feature] Keep voice sessions active across navigation`
