...
状态：已完成并合入 `main`。本文档作为会话页 Web 体验的历史计划和回归规则保留；native mobile 后续工作 MUST 以 `docs/native-mobile-architecture-plan.md` 为主入口。
...
本阶段已完成并合入 `main`。原集成目标是 **Phase 1 + Phase 4 基础版**，用于先交付可验收的沉浸式会话界面和字幕/布局体验。
...
Phase 2 + Phase 3 的基础音频驱动能力已完成并合入 `main`，原目标包括：

- Phase 2：listening 状态下，如果浏览器已授权麦克风且当前仍在 active `/session` turn 中，使用 Web Audio analyser 采样实时输入音量驱动 waveform。
- Phase 3：speaking 状态下，如果真实 TTS provider 返回可播放 `audioUrl`，使用 media element analyser 采样播放音量驱动 waveform。
- 如果权限、浏览器能力、provider 音频或跨域限制导致 analyser 不可用，MUST 回退到 Phase 1 的状态驱动 waveform。
- 本增强 MUST 不新建第二套用户可见麦克风权限流程，不后台采集麦克风，不发送音频原始数据到服务端。
- 本增强 MUST 不改变“用户真实输入 -> AI 回复 -> TTS 播放完成 -> 下一轮 listening”的核心时序。
...
- 后续增强分支 #52 MUST 实现基础版：仅在已授权、active `/session` listening turn 中采样，失败回退状态驱动 waveform。
...
- 保持 TTS 完播后再监听。
- 本次集成分支 MUST 不实现；仅保留为后续增强。
- 后续增强分支 #52 MUST 实现基础版：真实 TTS `audioUrl` 可分析时驱动 speaking waveform，mock/browser TTS 或 analyser 失败时回退状态驱动 waveform。
...
- 离开 `/session`、结束 session、STT 结束或播放完成时，MUST 停止 analyser、media tracks 和 animation frame。
